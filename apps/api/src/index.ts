...
// Serve local PDFs + fonts in development only. In production, pin
// storage to a real provider (R2/S3/GCS) via STORAGE_PROVIDER so
// PDFs and fonts get bucket-level access control instead of an
// unauthenticated localhost path.
if (process.env.NODE_ENV !== 'production') {
  ...
  // Local font handler. Each URL is HMAC-signed with an expiry, so
  // knowing the URL pattern + a victim's userId is not enough to
  // download their fonts. The signature is generated by
  // getFontCssForUser; the verifier here mirrors that logic.
  app.use('/fonts/*', async (c, next) => {
    const { verifyFontUrlSignature } = await import('./services/fonts.js');
    const path = c.req.path;
    const sig = c.req.query('sig');
    const expires = c.req.query('expires');
    if (!sig || !expires || !verifyFontUrlSignature(path, sig, expires)) {
      return c.json({ error: { code: 'FORBIDDEN', message: 'Invalid or expired font URL' } }, 403);
    }
    return next();
  });
  app.use(
