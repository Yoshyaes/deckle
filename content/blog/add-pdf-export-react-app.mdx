...
description: "Add a 'Download as PDF' button to your React app using Deckle's fromReact() API and @deckle/react component library."
...
Deckle gives you three ways to satisfy that request, and you can mix them within the same project:
...
2. **React component to PDF with `fromReact()`** -- send a JSX/TSX source string along with data as props. The component is rendered server-side by Deckle, giving you full control over the PDF layout without coupling it to your UI.
3. **`@deckle/react` component library** -- purpose-built components like `Document`, `Page`, `Table`, and `Watermark` that map directly to PDF concepts. Combine them with `fromReact()` for pixel-perfect results.
...
The simplest path is to grab HTML that is already on the page and send it to Deckle. This works well when your existing UI is close to what you want the PDF to look like.
...
## Using @deckle/react Components

For structured documents with recurring elements like headers, footers, tables, and watermarks, the `@deckle/react` component library provides pre-built primitives that handle PDF-specific concerns for you.

Install the package:

```bash
npm install @deckle/react
```
...
The button handles the loading state, error handling, and download trigger. When the user clicks, it sends the data to your API route, receives the PDF URL from Deckle, and initiates the browser download.

## Styling for PDF

PDF rendering differs from screen rendering in a few important ways. Deckle uses Chromium under the hood, so standard CSS works, but there are guidelines to follow for reliable output.
...
Here is a full working example that ties everything together. A dashboard page displays metrics and a data table on screen, and a button exports the same data as a professionally formatted PDF using `@deckle/react` components.
...
import React from "react";
import { Document, Page, Header, Footer, Table, Grid } from "@deckle/react";
      ...
      <Page size="A4" margin="20mm">
        ...
        <Footer style={{ borderTop: "1px solid #e5e5e5", paddingTop: "8px", fontSize: "10px", color: "#999" }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <span>Deckle Dashboard Export</span>
          </div>
        </Footer>
      </Page>
    </Document>
...
When the user clicks "Export Report," the button component sends the metrics and transaction data to the API route, which passes everything to Deckle via `fromReact()`. Deckle renders the `@deckle/react` components into a structured PDF with a branded header, metric cards in a grid, a striped and bordered data table, and a footer -- all generated in under two seconds.

## Going Further

You now have the tools to add PDF export to any React application, from a quick HTML capture to a fully designed document with `@deckle/react` components. Here are some next steps to explore:

- **[Generating PDFs in Next.js with Server Actions](https://getdeckle.dev/blog/generate-pdfs-nextjs-deckle)** -- deeper integration with the App Router and Server Actions pattern.
- **[Page Layout and Multi-Page Documents](https://getdeckle.dev/blog/pdf-page-breaks-headers-footers-guide)** -- advanced control over page breaks, headers that repeat across pages, and landscape orientation.
...
Check out the [Deckle documentation](https://getdeckle.dev/docs) for the full API reference and more examples.
