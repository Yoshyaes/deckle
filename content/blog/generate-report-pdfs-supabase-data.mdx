...
description: "Query your Supabase database, transform the data, and generate polished PDF reports with Deckle. Complete with tables, QR codes, and automatic scheduling."
...
In this tutorial, you will build a complete reporting pipeline that queries sales data from Supabase, feeds it into a Handlebars template with tables and QR codes, generates a pixel-perfect PDF with Deckle, stores the result back in Supabase, and automates the whole thing with an Edge Function. You will also see how to scale to multi-client batch reports using the Deckle batch API.

## Step 1: Set Up the Project

Start by creating a new Node.js project and installing the two dependencies you need: the Deckle SDK for PDF generation and the Supabase client for database queries.

```bash
mkdir sales-report-generator
cd sales-report-generator
npm init -y
npm install @getdeckle/sdk @supabase/supabase-js
```
...
The QR code at the bottom uses Deckle's `{{qr:data}}` placeholder syntax. Before rendering, Deckle replaces this with an inline SVG. The QR code links to your live dashboard URL, so anyone who receives the printed report can scan it to see real-time data. This is particularly useful for reports that are printed and distributed in meetings.
...
Now you can query the `reports` table to show a history of generated reports, link to the PDF URLs, or track generation performance over time. This is also useful for debugging: if a stakeholder says a report looks wrong, you can look up the exact generation ID and inspect it in your Deckle dashboard.
...
serve(async (req) => {
  try {
    ...
    // Call Deckle API directly (no npm SDK in Deno)
    const apiKey = Deno.env.get("DECKLE_API_KEY")!;
    const templateId = Deno.env.get("REPORT_TEMPLATE_ID")!;

    const generateResponse = await fetch("https://api.deckle.com/v1/generate", {
      method: "POST",
      headers: {
...
supabase secrets set DECKLE_API_KEY=dk_live_your_key
...
If you run a multi-tenant platform, you need to generate separate reports for each client. Doing this sequentially is slow. The Deckle batch API lets you queue all reports at once and get notified when they are done.
...
The `df.batch()` call returns immediately with HTTP 202. The response includes a `batch_id`, the `total` number of reports queued, and an array of `generations` with each item's `id` and `index`. Deckle processes the jobs concurrently on its side using a BullMQ worker pool with retries and exponential backoff.

When all reports finish, Deckle sends a POST request to your webhook URL. Your webhook handler can then update the batch status in Supabase, notify clients via email, or trigger downstream workflows. For a batch of 50 client reports, this approach is dramatically faster than generating them one at a time.
...
- **[Automate Stripe invoices](https://getdeckle.dev/blog/invoice-system-stripe-deckle)** -- Generate PDF invoices on every successful Stripe payment and email them to customers.
- **[Generate PDFs in Next.js](https://getdeckle.dev/blog/generate-pdfs-nextjs-deckle)** -- Build a dashboard front end that lets users trigger report generation on demand.
- **[Page layout guide](https://getdeckle.dev/blog/pdf-page-breaks-headers-footers-guide)** -- Fine-tune margins, headers, footers, and multi-page layouts for professional documents.

Check out the [full API reference](https://getdeckle.dev/docs/api-reference) for additional features like PDF merging, splitting, password protection, and form filling.
