---
title: "Build an Invoice System with Stripe + Deckle"
description: "Wire up Stripe payment events to automatically generate professional PDF invoices with Deckle. Complete webhook handler and template included."
slug: "invoice-system-stripe-deckle"
date: "2026-03-14"
author: "Deckle Team"
...
That is where Deckle comes in. Deckle is a PDF generation API that accepts HTML templates and structured data, then returns pixel-perfect PDFs. By wiring Stripe payment events to Deckle templates, you get a fully automated invoice pipeline: a customer pays, Stripe fires a webhook, your server maps the payment data into a Handlebars template, Deckle renders the PDF, and you email it to the customer. No manual steps, no cron jobs, no brittle HTML-to-PDF libraries running on your own infrastructure.

In this tutorial, you will build exactly that system. By the end, you will have a production-ready webhook handler that listens for `invoice.payment_succeeded` events from Stripe, generates a branded PDF invoice through Deckle, and emails it to the customer using Resend. You will also learn how to use batch generation for monthly billing runs. The full source code is included at the end.

**What you will need:**

- A Stripe account with a webhook endpoint configured
- A Deckle API key (sign up at deckle.com)
...
The tech stack is deliberately minimal. You will use Next.js API routes for the webhook handler, the official Stripe Node SDK for signature verification and data extraction, the Deckle TypeScript SDK for PDF generation, and Resend for transactional email. If you prefer Express or Fastify, the core logic is identical -- only the route definition changes.

All communication is synchronous except for the optional batch generation path, which uses Deckle's async queue and webhook callbacks.

## Step 1: Create the Invoice Template

Before handling any Stripe events, you need an invoice template stored in Deckle. Templates use Handlebars syntax for dynamic data and standard HTML/CSS for layout. Once created, you reference the template by ID and pass in different data for each invoice.

Install the dependencies first:

```bash
npm install @getdeckle/sdk stripe resend
```
...
The response includes everything you need: a hosted URL where the PDF can be downloaded, the page count, file size, and how long the render took. Deckle hosts the file for you, so there is no need to set up your own storage bucket unless you want to.

The footer uses the `{{pageNumber}}` and `{{totalPages}}` interpolation variables that Deckle injects automatically. These work independently of the Handlebars template variables in the body -- they are processed by the PDF renderer at print time.
...
If you run a subscription business, you might need to generate dozens or hundreds of invoices at the end of each billing cycle. Generating them one by one would be slow and wasteful. Deckle's batch endpoint handles this efficiently -- you submit all invoices in a single request, and they are processed concurrently on Deckle's infrastructure.
...
async function generateMonthlyInvoices(
  ...
  const batch = await df.batch({
    items: batchItems,
    webhook: "https://your-app.com/api/webhooks/deckle-batch",
  });
...
There are two ways to know when the batch is complete. The recommended approach is the webhook callback -- Deckle will POST to your webhook URL when all items are finished. Alternatively, you can poll individual generations:
...
For **reporting**, look at our guide on [generating Supabase reports with Deckle](https://getdeckle.dev/blog/generate-report-pdfs-supabase-data) -- the same template and batch patterns apply to monthly financial summaries.

If you want more control over **page layout** -- custom headers per page, landscape tables, or page break control -- the [page layout and headers guide](https://getdeckle.dev/blog/pdf-page-breaks-headers-footers-guide) covers all the options available in the `PDFOptions` object.

For teams building a full SaaS billing portal, the [Next.js integration guide](https://getdeckle.dev/docs/guides/nextjs) shows how to add a customer-facing invoice history page with on-demand PDF regeneration.
