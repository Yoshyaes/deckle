...
slug: "generate-pdfs-nextjs-deckle"
date: "2026-03-10"
author: "Deckle Team"
...
In this tutorial, you will build a complete PDF generation flow in a Next.js 14 application using Deckle. By the end, you will have an API route that generates PDFs from raw HTML, a reusable template system with Handlebars, headers and footers with page numbers, and a clean client-side download button.
...
- A **Deckle account** and API key (grab one from the [dashboard](https://app.getdeckle.dev/keys))
- **Node.js 18** or later
...
Install the `deckle` npm package in your Next.js project:
...
The `df.generate()` call sends your HTML to Deckle's rendering pipeline, which uses a headless Chromium instance to produce a pixel-perfect PDF. The response includes everything you need:

```json
{
  "id": "gen_abc123xyz",
  "status": "completed",
  "url": "https://storage.deckle.com/pdfs/gen_abc123xyz.pdf",
  "pages": 1,
  "file_size": 48210,
...
Notice that the Deckle client is instantiated outside the handler. This avoids creating a new client on every request and keeps the connection efficient.
...
```tsx
<PdfDownloadButton
  html="<h1>Hello from Deckle</h1><p>This is a generated PDF.</p>"
...
Hardcoding HTML strings works for prototyping, but production applications need reusable templates. Deckle templates use Handlebars syntax, which gives you variables, loops, and conditionals without the overhead of a full templating engine on your side.
...
Templates also support versioning. Every time you update a template through the API or dashboard, Deckle saves the previous version. You can list versions, view any historical snapshot, and restore if something goes wrong. This is critical for compliance-heavy documents where you need to prove what a template looked like on a specific date.

## Step 5: Add Headers, Footers, and Page Numbers

Multi-page documents like contracts and reports need consistent headers and footers. Deckle supports HTML-based headers and footers with two special interpolation variables: `{{pageNumber}}` and `{{totalPages}}`.
...
- **[Automate Stripe invoices](https://getdeckle.dev/blog/invoice-system-stripe-deckle)** -- Generate and email PDF invoices on every successful payment using Stripe webhooks.
- **[Export React components to PDF](https://getdeckle.dev/blog/add-pdf-export-react-app)** -- Use `df.fromReact()` to render your existing React components directly to PDF without maintaining separate HTML templates.
- **[Deckle vs Puppeteer](https://getdeckle.dev/blog/deckle-vs-puppeteer-why-i-switched)** -- Understand the tradeoffs between a managed API and self-hosted browser automation.
- **[Generate PDFs in Express](https://getdeckle.dev/blog/express-pdf-api-html-to-pdf)** -- If your backend is not Next.js, the same SDK works in any Node.js environment.

Check out the [full API reference](https://getdeckle.dev/docs/api-reference) for advanced features like batch generation, webhook callbacks, and PDF tools including merge, split, and password protection.
