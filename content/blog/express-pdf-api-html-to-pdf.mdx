...
description: "Build a complete PDF generation API with Express.js and Deckle in under a minute. HTML-to-PDF, templates, downloads, and error handling included."
...
Deckle eliminates that entire layer. You send HTML (or a template, or JSX) to a single API call and get a production-ready PDF back. No browser binaries. No process management. No memory leaks at 3 AM.
...
Add your Deckle API key to a `.env` file. Never commit this file to version control.

```bash
DECKLE_API_KEY=dk_live_your_api_key_here
```

Now initialize the Deckle client. Create `src/server.ts` and add the foundation:
...
Run it with `npx tsx --env-file=.env src/server.ts`. You have a running Express server with a configured Deckle client. Time to add endpoints.
...
The `{{pageNumber}}` and `{{totalPages}}` tokens are interpolated automatically by Deckle during rendering.

## Step 3: Template-Based Invoice Endpoint

Raw HTML works for one-off documents, but most real applications generate PDFs from structured data. Invoices, receipts, reports -- these all follow a fixed layout populated with dynamic values. Deckle templates use Handlebars syntax (`{{variable}}`, `{{#each items}}`, `{{#if}}`) and are stored server-side with version history.

Create a template in the Deckle dashboard first (or via the API), then reference it by ID:
...
Templates also support version history. Every update to a template creates a new version, so you can roll back if a layout change breaks something. The template ID stays the same across versions -- Deckle always uses the latest version unless you specify otherwise.
...
This is faster since the PDF is generated and stored on Deckle's CDN, and the browser fetches it directly. The tradeoff is that the URL is temporarily public. For sensitive documents like invoices, the base64 approach keeps the PDF entirely within your server's control.

## Step 5: Error Handling Middleware

Deckle's SDK throws typed errors that map cleanly to HTTP status codes. A single Express error middleware can catch them all and return consistent responses:
...
app.use(
  (
    ...
    if (err instanceof AuthenticationError) {
      res.status(401).json({
        error: "Invalid or missing Deckle API key",
      });
      return;
    }
...
The order matters. `RateLimitError` and `AuthenticationError` extend `DeckleError`, so they must be checked first. If you catch `DeckleError` at the top, the specific subclasses would never match.
...
The `ValidationError` covers cases like malformed HTML, missing template variables, or invalid PDF options. The error message from Deckle is descriptive enough to return directly to the caller.

Because every route uses `next(err)` in its catch block, all Deckle errors flow through this single middleware. You write the error mapping once and every endpoint benefits.

## Step 6: React-to-PDF Endpoint

If your team writes components in React, you can skip the HTML string entirely and send JSX to Deckle. The server transpiles it, renders it to static markup, and converts it to a PDF:
...
The `react` field accepts a JSX/TSX string. Deckle transpiles it with esbuild on the server, so you do not need a build step. The optional `data` object is passed as props to the root component, and `styles` accepts a CSS string applied to the rendered output.
...
- **Add authentication middleware** to protect your endpoints and pass user context to Deckle for usage tracking.
...
For more depth, see the [Next.js PDF generation guide](https://getdeckle.dev/blog/generate-pdfs-nextjs-deckle) for framework-specific patterns, the [Deckle vs Puppeteer comparison](https://getdeckle.dev/blog/deckle-vs-puppeteer-why-i-switched) for architectural context, and the [page layout and styling guide](https://getdeckle.dev/blog/pdf-page-breaks-headers-footers-guide) for advanced CSS techniques in PDF rendering.
