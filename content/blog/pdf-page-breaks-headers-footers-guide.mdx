...
description: "Master CSS page breaks, repeating headers and footers, page numbers, and multi-page layout for pixel-perfect PDFs with Deckle."
...
## Headers via Deckle Options

Deckle supports repeating headers on every page of your PDF through the `options.header` property. This header renders inside the top margin area and supports dynamic page number interpolation.
...
Use `{{pageNumber}}` and `{{totalPages}}` inside your header HTML. Deckle interpolates these values for each page during rendering.
...
If you prefer a component-based workflow, the `@deckle/react` library provides `Document`, `Page`, `Header`, and `Footer` components that handle page breaks and positioning for you.
...
```tsx
import { Document, Page, Header, Footer } from "@deckle/react";
...
**External CSS in headers and footers.** Stylesheet links and `<style>` blocks do not work inside `options.header` and `options.footer`. All styling must be inline. This is a Chromium limitation, not a Deckle limitation.
...
- [Generate PDFs in Next.js with Deckle](https://getdeckle.dev/blog/generate-pdfs-nextjs-deckle) for a complete Next.js integration
- [Build a Stripe Invoice System](https://getdeckle.dev/blog/invoice-system-stripe-deckle) for multi-page invoice generation with page numbers
- [Generate Report PDFs from Supabase Data](https://getdeckle.dev/blog/generate-report-pdfs-supabase-data) for data-driven flowing reports
- [Python PDF Generation with FastAPI](https://getdeckle.dev/blog/pdf-generation-python-fastapi-deckle) for server-side generation in Python
